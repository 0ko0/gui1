...
local function ProtectGui(gui)
    ...
    pcall(function()
        hidden_container = (gethui and gethui()) or (get_hidden_gui and get_hidden_gui())
    end)
    ...
    if hidden_container then
        ...
        local successRobloxGui = pcall(function()
            ...
            if robloxGui then
                gui.Parent = safe_cloneref(robloxGui)
            else
                error()
            end
        end)
        ...
        if not successRobloxGui then
            ...
            if not successCore then
                local playerGui = safe_cloneref(Players.LocalPlayer:WaitForChild("PlayerGui"))
                gui.Parent = playerGui
                warn("[Library] Warning: Your executor does not support gethui(), the UI is currently in a state that MAY BE DETECTED by Anti-Cheat")
            end
        end
    end
end
...
function library:ToggleUI(keybind)
	...
	if inputService.TouchEnabled or true then 
		...
		mainContainer.Name = "ToggleContainer"
		mainContainer.Position = UDim2.new(0, 20, 0.5, -25)
		mainContainer.Size = UDim2.new(0, 46, 0, 46)
		mainContainer.BackgroundTransparency = 1
		mainContainer.Parent = toggleGui
				...
				if not hasDragged then
					...
					task.delay(0.2, function()
						...
						tweenService:Create(icon, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
							Rotation = 0, ImageTransparency = 0, Size = UDim2.new(0, 22, 0, 22)
						}):Play()
					end)
				end
